...
export RCM_DEPLOY_COMMAND="python ${RCM_DEPLOY_ROOTPATH}/scripts/deploy_setup.py --integration --clearconfig --debug=debug  --branches clean/develop     pr/.*  wip/.* --prlist $PR_NEEDED $PR_CLIENT_NEEDED $PR_UTILS --dest $(pwd)/spack  --config ${RCM_DEPLOY_HOSTPATH} --install $(pwd)/install" 
