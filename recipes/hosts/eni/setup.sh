...
export CONFIG_DIRS="config config/shared_install config/rcm config/rcm/server config/rcm/server/gpu config/insitu "
echo "-->$CONFIG_DIRS<--"
if [ "x$1" != "x" ]
then
  ...
  if [ "$1" == "dev_all" ]
  then
    export CONFIG_DIRS="$CONFIG_DIRS config/openfoam config/meteo config/rcm/develop"
  fi  
